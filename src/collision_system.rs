...
pub mod QuadCollider;
pub mod TriangleCollider;
...
trait PolyCollision {
    ...
    fn is_collision(&self, other: &impl PolyCollision) -> (bool, Vector2<f32>)
    where
        Self: std::marker::Sized,
    {
        let mut edges = self.edges_of();
        ...
        for (_i, v) in edges.iter().enumerate() {
            orthogonals.push(Self::otho(v));
        }
        let mut push_vectors: Vec<Vector2<f32>> = Vec::new();
        for o in &orthogonals {
            let (separated, pv) = Self::is_separating_axis(o, self, other);
            if separated {
        ...
        let mpv = Self::min_vec(&push_vectors);
        (true, mpv)
    }
